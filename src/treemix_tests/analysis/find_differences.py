...
class TreeComparator:
    ...
    def __init__(
        ...
        tree2: Tree,
        output_dir: str,
        migration_edges: Dict[str, List[Dict[str, Any]]],
        basename: str = "",
    ):
        """
    ...
    def _compare_siblings(self, common_taxa: Set[str]) -> Dict[str, Any]:
        ...
        return {"different_populations": different_pops, "comparisons": sibling_comparisons}
    ...
    def _get_sibling_taxa(tree: Tree, taxon_label: str) -> Set[str]:
        ...
        return {
            leaf.taxon.label
            for leaf in node.parent_node.leaf_nodes()
            if leaf.taxon.label != taxon_label
        }
    ...
    @staticmethod
    def _compute_z_score(dist1: Dict[str, float], dist2: Dict[str, float]) -> Dict[str, Any]:
        ...
        return {"mean_delta": mean_delta, "std_delta": std_delta, "differences": differences}


def load_trees(
    baseline_path: str, tree_path: str
) -> Tuple[Tree, Tree, Dict[str, List[Dict[str, Any]]]]:
    """
...
def extract_migration_edges_from_treeout(file: str) -> List[Dict[str, Any]]:
    ...
    for line in lines:
        ...
        try:
            ...
            migration_edges.append({"weight": weight, "source": source_taxa, "target": target_taxa})
...
def main(
    baseline_path: str, tree_path: str, output_dir: str, basename: str, pruned: bool = False
) -> None:
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        "-o <output_directory> -b <basename>",
        description="A program to compare the experiments tree generated to the baseline trees.",
    )
    parser.add_argument(
        "-i", dest="input_file", help="Enter the path of the baseline treemix output file."
    )
    parser.add_argument(
        "-c", dest="comparison_tree", help="Enter the path of the tree you wich to compare."
    )
    parser.add_argument(
        "-o", dest="output_dir", help="Enter the output directory to store the results."
    )
