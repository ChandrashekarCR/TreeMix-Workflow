...
plot_tree_internal = function(d, e, o = NA, cex = 1, disp = 0.005, plus = 0.005, arrow = 0.05, ybar = 0.01, scale = T, mbar = T, mse = 0.01, plotmig = T, plotnames = T, xmin = 0, lwd = 1, font = 1, base_name="base", mark_changes = FALSE, changed_pops_path = NULL ){
  plot(d$x, d$y, axes = F, ylab = "", xlab = "Drift parameter", main=base_name, xlim = c(xmin, 0.15), pch = "",cex.lab = 0.7) #c(xmin, max(d$x)+plus), pch = "",  ) # added cex.lab = 0.7,
	#axis(1)
  par(mar = c(5, 4, 3, 2))  # Fixe Margins: unten, links, oben, rechts
  axis(1, 
       at = seq(0, 0.15, by = 0.01), 
	...
	if ( !is.na(o)){
		for(i in 1:nrow(tmp)){
			...
			if(plotnames){
				#print(tmp[i,2])
			  #text_color = ifelse(node_name %in% names(color_dict), color_dict[[node_name]], "black")
		...
		if (plotnames){
		  ...
		  # Generate a named vector of colors using RColorBrewer or another palette
		  library(RColorBrewer)
		  palette <- brewer.pal(n = length(superpops), name = "Dark2")  # or Set3, Dark2, etc.
		  ...
		  for (i in 1:nrow(tmp)) {
		    ...
		    # Get the continent
		    continent = ifelse(node_name %in% names(CONTINENT_MAP), CONTINENT_MAP[[node_name]], "Unknown")
		    
		    # Get the color (default to black if not found)
		    ...
		    label_height <- strheight(node_name, cex = cex, font = 4)
		    horizontal_padding <- 0.01  # previously might have been 0.03 or larger
		    ...
		    rect(
		      ...
		      border = rgb(1,1,1, alpha = 0),
		      col = rgb(1,1,1, alpha = 0)  # fully transparent fill
		    )
		    ...
		    text(tmp[i,]$x + disp, tmp[i,]$y, labels = node_name, adj = 0, 
		         cex = cex, col = text_color, font = font_weight)
		  }
		} 
...
plot_tree = function(stem, o = NA, cex = 1, disp = 0.003, plus = 0.01, flip = vector(), arrow = 0.05, scale = T, ybar = 0.1, mbar = T, plotmig = T, plotnames = T, xmin = 0, lwd = 1, font = 1, base_name="base", mark_changes_bool, changed_pops){
	...
	print(base_name)
	plot_tree_internal(d, e, o = o, cex = cex, xmin = xmin, disp = disp, plus = plus, arrow = arrow, ybar = ybar, mbar = mbar, mse = m, scale = scale, plotmig = plotmig, plotnames = plotnames, lwd = lwd, font = font, base_name=base_name, mark_changes= mark_changes_bool,changed_pops_path = changed_pops)
	return(list( d= d, e = e))
}
