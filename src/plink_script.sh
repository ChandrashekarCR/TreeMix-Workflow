...
PLINK2TREEMIX="$PROJECT_ROOT/src/treemix_tests/plink2treemix.py"
POPMANIPULATION="$PROJECT_ROOT/src/treemix_tests/population_manipulation.py"
POP_MAPPING="$PROJECT_ROOT/raw_data/region_mapping.json"
...
POP_LIST="$PROJECT_ROOT/raw_data/2306_poplist.tsv"
...
# Experiment 2: Genotype Missingness
# ---------------------------------------------------
...
test_2() { 
    # Creates results directory
    echo "===== Running Experiment 2: Genotype Missingness ===== "

    # Creates results directory
    OUT_DIR="$RESULTS_DIR/experiment_2/plink_results"
    mkdir -p $OUT_DIR
...
# Experiment 3: Minor Allele Frequency (MAF)
# ---------------------------------------------------
...
test_3() {
    # Creates results directory
    echo "===== Running Experiment 3: Minor Allele Frequency ===== "
    OUT_DIR="$RESULTS_DIR/experiment_3/plink_results"
    mkdir -p $OUT_DIR
    ...
    echo "Experiment 3 completed. Results in: $OUT_DIR"
}


# Experiment 4: Drop Populations Specific Populations
# ---------------------------------------------------
...
test_4() {
    echo "===== Running Experiment 4: Drop specific populations  ===== "
...
# Experiment 5: Hominins Outgroup
# ---------------------------------------------------
...
test_5() {
    ...
    # Creates results directory
    echo "===== Running Experiment 5: Hominins Outgroup ===== "
    OUT_DIR="$RESULTS_DIR/experiment_5/plink_results"
    mkdir -p $OUT_DIR

    # Function to process a hominin population (Denisovan or Vindija)
    process_hominin() {
        local POP_NAME=$1      # "Deni" or "Vindija"
        local PREFIX="$OUT_DIR/5_added_${POP_NAME}"
    ...
    echo "Experiment 5 completed. Results in: $OUT_DIR"
}



# Experiment 6: Uneven Sample Number in Specific Populations
# ---------------------------------------------------
...
test_6() {
    echo "===== Running Experiment 6: Uneven Sample Number in Specific Populations ===== "
    OUT_DIR="$RESULTS_DIR/experiment_61/plink_results"
    mkdir -p $OUT_DIR
    ...
    for MAX in "${MAX_PER_POP[@]}"; do
        ...
        PREFIX_OUT="${OUT_DIR}/6_filtered_${MAX}"
    ...
    echo "Experiment 6 completed. Results in: $OUT_DIR"
}


# Experiment 7: Population Subsampling (Even Sample Number)
# ---------------------------------------------------
...
#
test_7() {
    echo "===== Running Experiment 7: Even the sample number ===== "
    OUT_DIR="$RESULTS_DIR/experiment_7/plink_results"
    mkdir -p $OUT_DIR
    ...
    echo "Experiment 7 completed. Results in: $OUT_DIR"
}



# Experiment 8: Remove Continental Populations
# ---------------------------------------------------
...
#
test_8() {
    echo "===== Running Experiment 8: Remove continental populations ===== "
    ...
    for continent in "${continents[@]}"; do
        ...
        PREFIX="$OUT_DIR/8_dataset_no_${safe_continent}"
...
# Experiment 9: Drop Specific Historically Admixed Populations
# ---------------------------------------------------
...
test_9() {
    echo "===== Running Experiment 9: Drop specific historical admixed populations ===== "
    OUT_DIR="$RESULTS_DIR/experiment_9/plink_results"
    mkdir -p $OUT_DIR
    PREFIX="$OUT_DIR/9_drop_admixed_pops"
    ...
    echo "Experiment 9 completed. Results in: $OUT_DIR"
}

# Experiment 10: Add Artificial hybrid populations:
# ---------------------------------------------------
...
test_10() {
    echo "===== Running Experiment 10: Artificial hybrid populations with two-mixtures ===== "
    OUT_DIR="$RESULTS_DIR/experiment_10/plink_results"
    mkdir -p $OUT_DIR
...
#    # Step 2: Create one big PED with all hybrids
    ...
    $PLINK --bfile "$OUT_DIR/all" --bmerge "$HYBRID_DIR/all_hybrids" --make-bed --out "$OUT_DIR/10_artificial_2_merged_all"

    ### Step 5: Run TreeMix input converter
    convert_plink_to_treemix "$OUT_DIR/10_artificial_2_merged_all" "$OUT_DIR" "$LIST_DIR/population_list_with_hybrids.tsv"


    echo "Experiment 10 completed. Results in: $OUT_DIR"
}


# Experiment 11: Artificial hybrid populations with five-mixtures:
# ---------------------------------------------------
...
test_11() {
    echo "===== Running Experiment 11: Artificial hybrid populations with two-mixtures ===== "
    OUT_DIR="$RESULTS_DIR/experiment_11/plink_results"
    mkdir -p $OUT_DIR
...
#    # Step 2: Create one big PED with all hybrids
    ...
    $PLINK --bfile "$OUT_DIR/all" --bmerge "$HYBRID_DIR/all_hybrids" --make-bed --out "$OUT_DIR/11_artificial_5_merged_all"

    ### Step 5 (optional): Run TreeMix input converter
    convert_plink_to_treemix "$OUT_DIR/11_artificial_5_merged_all" "$OUT_DIR" "$LIST_DIR/population_list_with_hybrids.tsv"


    echo "Experiment 11 completed. Results in: $OUT_DIR"
}
